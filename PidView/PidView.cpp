...
void PidView::selectionChanged()                     //点选曲线或图例
{
...
void PidView::contextMenuRequest(QPoint pos)         //鼠标右键--图像复位
{
	Q_UNUSED(pos);
...
void PidView::ReplotGraphSlot()                           //定时器图形刷新
{
	PidViewGraph();
	ui.customPlot->replot();
}
void PidView::RefreshAxisSlot()                      //因为共用一个横轴的原因 所以要实时更新坐标轴信息
{
...
void PidView::onCheckBoxSet(int i)                     //点击CheckBox 数据配置列表 产生信号对应的槽
{
